...
class CEmit
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(null != spec);
	    CUtility.jlexassert(null != outstream);
	  }
  ...
  /*void emit_imports
      ...
      throws java.io.IOException      
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.jlexassert(null != m_spec);
	      CUtility.jlexassert(null != m_outstream);
	    }*/
	  ...
	  while (states.hasMoreElements())
	    {
	      ...
	      if (CUtility.DEBUG)
		{
		  CUtility.jlexassert(null != state);
		}
  ...
  private void emit_table
	...
	for (elem=0; elem<size; elem++) {
	    ...
	    CUtility.jlexassert(dtrans.m_dtrans.length==m_spec.m_dtrans_ncols);
  ...
  private void emit_actions 
      ...
      throws java.io.IOException
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.jlexassert(m_spec.m_accept_vector.size() 
...
class CMakeNfa
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(null != input);
	    CUtility.jlexassert(null != lexGen);
	    CUtility.jlexassert(null != spec);
	  }
  ...
  void allocate_BOL_EOF
	...
	{
	  CUtility.jlexassert(CSpec.NUM_PSEUDO==2);
...
class CMinimize
{
  ...
  private void reduce
	...
	for (reduced_ncols = 0; ; ++reduced_ncols)
	  {
		...
		for (i = 0; i < reduced_ncols; ++i)
		  {
		    CUtility.jlexassert(-1 != m_spec.m_col_map[i]);
		  }
	      }
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.jlexassert(false == set.get(i));
		CUtility.jlexassert(-1 == m_spec.m_col_map[i]);
	      }
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(k == reduced_ncols);
	  }
	...
	for (reduced_nrows = 0; ; ++reduced_nrows)
	  {
		...
		for (i = 0; i < reduced_nrows; ++i)
		  {
		    CUtility.jlexassert(-1 != m_spec.m_row_map[i]);
		  }
	      }
	...
	if (CUtility.DEBUG)
	  {
	    ...
	    CUtility.jlexassert(k == reduced_nrows);
	  }
      }
	...
	while (old_group_count != group_count)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.jlexassert(m_group.size() == group_count);
	      }
		...
		for (j = 1; j < group_size; ++j)
		  {
			...
			if (goto_first != goto_next
				...
				|| m_ingroup[goto_next] != m_ingroup[goto_first]))
			  {
			    if (CUtility.DEBUG)
			      {
				CUtility.jlexassert(dtrans_group.elementAt(j) == next);
			      }
			    ...
			    if (CUtility.DEBUG)
			      {
				CUtility.jlexassert(m_group.contains(new_group)
						== true);
				CUtility.jlexassert(m_group.contains(dtrans_group)
						== true);
				CUtility.jlexassert(dtrans_group.contains(first)
						== true);
				CUtility.jlexassert(dtrans_group.contains(next)
						== false);
				CUtility.jlexassert(new_group.contains(first)
						== false);
				CUtility.jlexassert(new_group.contains(next)
						== true);
				CUtility.jlexassert(dtrans_group.size() == group_size);
				CUtility.jlexassert(i == m_ingroup[first.m_label]);
				CUtility.jlexassert((m_group.size() - 1) 
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.jlexassert(i == dtrans.m_label);
		CUtility.jlexassert(false == group_found);
		CUtility.jlexassert(group_count == m_group.size());
	      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.jlexassert(false == group_found);
		    CUtility.jlexassert(0 < dtrans_group.size());
		  }
		...
		if (CUtility.SLOW_DEBUG)
		  {
		    ...
		    s = dtrans_group.size();
		    CUtility.jlexassert(0 < s);

		    for (k = 1; k < s; ++k)
		      {
			check = (CDTrans) dtrans_group.elementAt(k);
			CUtility.jlexassert(check.m_accept == first.m_accept);
		      }
		  }
		...
		if (first.m_accept == dtrans.m_accept)
		  {
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.jlexassert(j == m_ingroup[dtrans.m_label]);
		      }
...
class CNfa2Dfa
{
	...
	for (istate = 0; nstates > istate; ++istate)
	  {
	    ...
	    */
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.jlexassert(false == dfa.m_mark);
		  }
		...
		for (i = 0; i < m_spec.m_dtrans_ncols; ++i)
		  {
		    if (CUtility.DEBUG)
		      {
			CUtility.jlexassert(0 <= i);
			CUtility.jlexassert(m_spec.m_dtrans_ncols > i);
		      }
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.jlexassert((null == bunch.m_nfa_set 
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.jlexassert(nextstate < m_spec.m_dfa_states.size());
		      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.jlexassert(m_spec.m_dtrans_vector.size() == dfa.m_label);
		  }
  ...
  private void e_closure
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(null != bunch);
	    CUtility.jlexassert(null != bunch.m_nfa_set);
	    CUtility.jlexassert(null != bunch.m_nfa_bit);
	  }
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.jlexassert(bunch.m_nfa_bit.get(state.m_label));
	      }
	...
	while (false == nfa_stack.empty())
	  {
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.jlexassert(null != bunch.m_accept);
		    CUtility.jlexassert(CSpec.NONE == bunch.m_anchor
		...
		if (null != state.m_next)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.jlexassert(false == bunch.m_nfa_bit.get(state.m_next.m_label));
			  }
		...
		if (null != state.m_next2)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.jlexassert(false == bunch.m_nfa_bit.get(state.m_next2.m_label));
			  }
...
class CLexGen 
{
  ...
  void generate
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(null != this);
	    CUtility.jlexassert(null != m_outstream);
	    CUtility.jlexassert(null != m_input);
	    CUtility.jlexassert(null != m_tokens);
	    CUtility.jlexassert(null != m_spec);
	    CUtility.jlexassert(m_init_flag);
	  }
  ...
  private char[] packCode
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(INIT_CODE == specified 
  ...
  private void userRules
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(END_OF_INPUT == m_spec.m_current_token);
	  }
	...
	while (states.hasMoreElements())
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.jlexassert(null != state);
		CUtility.jlexassert(null != index);
	      }
  ...
  private boolean expandMacro
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(0 < count_name);
	  }
	...
	for (rep_elem = 0; rep_elem < start_macro; ++rep_elem)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.jlexassert(rep_elem < replace.length);
	      }
	  }
  ...
  private void saveMacro
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(0 < count_def);
	    CUtility.jlexassert(0 < count_name);
	    CUtility.jlexassert(null != m_spec.m_macros);
	  }
  ...
  private void saveStates
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert('%' == m_input.m_line[0]);
	    CUtility.jlexassert('s' == m_input.m_line[1]);
	    CUtility.jlexassert(m_input.m_line_index <= m_input.m_line_read);
	    CUtility.jlexassert(0 <= m_input.m_line_index);
	    CUtility.jlexassert(0 <= m_input.m_line_read);
	  }
  ...
  CAccept packAccept
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.jlexassert(null != accept);
	  }
  ...
  private void details
	...
	while (names.hasMoreElements())
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.jlexassert(null != name);
		CUtility.jlexassert(null != def);
	      }
	...
	if (false == m_spec.m_count_chars)
	...
	else
	  {
	    if (CUtility.DEBUG)
	      {
		CUtility.jlexassert(m_spec.m_count_lines);
	      }
...
final class SparseBitSet implements Cloneable {
	...
	CUtility.jlexassert(size<bits.length);
	insert_block(idx, bnum);
    }
    private void insert_block(int idx, int bnum) {
	CUtility.jlexassert(idx<=size);
	CUtility.jlexassert(idx==size || offs[idx]!=bnum);
	...
	CUtility.jlexassert(!a.get(0) && !a.get(1));
	CUtility.jlexassert(!a.get(123329));
	a.set(0); CUtility.jlexassert(a.get(0) && !a.get(1));
	a.set(1); CUtility.jlexassert(a.get(0) && a.get(1));
	a.clearAll();
	CUtility.jlexassert(!a.get(0) && !a.get(1));
	...
	for (int n=0; n<ITER; n++) {
	    ...
	    CUtility.jlexassert(a.get(rr) && !a.get(rr+1) && !a.get(rr-1));
	    for (int i=0; i<v.size(); i++)
		CUtility.jlexassert(a.get(((Integer)v.elementAt(i)).intValue()));
	}
	SparseBitSet b = (SparseBitSet) a.clone();
	CUtility.jlexassert(a.equals(b) && b.equals(a));
	for (int n=0; n<ITER/2; n++) {
	    ...
	    CUtility.jlexassert(!b.get(m));
	}
	CUtility.jlexassert(!a.equals(b));
	SparseBitSet c = (SparseBitSet) a.clone();
	SparseBitSet d = (SparseBitSet) a.clone();
	c.and(a);
	CUtility.jlexassert(c.equals(a) && a.equals(c));
	c.xor(a);
	CUtility.jlexassert(!c.equals(a) && c.size()==0);
	d.or(b);
	CUtility.jlexassert(d.equals(a) && !b.equals(d));
	d.and(b);
	CUtility.jlexassert(!d.equals(a) && b.equals(d));
	d.xor(a);
	CUtility.jlexassert(!d.equals(a) && !b.equals(d));
